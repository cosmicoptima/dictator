...
impersonateUser whoTo whereTo whatTo = do
  ...
  hook      <- case maybeHook of
    Just hook -> do
      restCall' . ModifyWebhook hook $ ModifyWebhookOpts (Just name)
                                                         mayAvatar
                                                         (Just whereTo)
    Nothing -> do
      hook <- restCall' . CreateWebhook whereTo $ CreateWebhookOpts name
                                                                    mayAvatar
...
fightTrinkets t1 t2 winner = do
  ...
  let mayResult =
        rightToMaybe . parse parTrinketCombat "" <=< listToMaybe . lines $ res
  case mayResult of
 ...
 where
  ...
  parTrinketCombat = do
    ...
    let
      t1Words =
        MS.fromList . filter ((> 3) . T.length) . words . view trinketName $ t1
      t2Words =
        MS.fromList . filter ((> 3) . T.length) . words . view trinketName $ t2
    void $ string ", because: "
...
getAction name = do
 ...
 where
  ...
  parser = do
    ...
    effect <- (string " [" *> actionParser `sepBy1` string ", " <* string "]")
      <|> return []
    return (desc, effect)
...
userOwns userData items =
  let
    ...
    ownsTrinkets = claimedTrinkets `MS.isSubsetOf` (userData ^. userTrinkets)
