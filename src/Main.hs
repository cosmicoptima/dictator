...
updateForbiddenWords = do
  ...
  rng <- newStdGen
  let
    warning = voiceFilter $ if odds 0.5 rng
      then
...
scheduledEvents =
  [ ScheduledEvent { absDelay = hours 2, scheduledEvent = updateForbiddenWords }
    ...
    flip runReaderT env . logErrorsInChannel channel $ do
      ...
      when ((emojiName . reactionEmoji) react `elem` birds) $ do
        ...
        let met_threshhold = length users >= 3
            ...
            not_tweeted    = message `Set.notMember` (global ^. globalTweeted)
